...
SHOPBOT_MARKUP_USDT = _dec_env("SHOPBOT_MARKUP_USDT", str(MARKUP_USDT))
...
SHOPBOT_MARKUP_PERCENT = _dec_env("SHOPBOT_MARKUP_PERCENT", str(MARKUP_PERCENT))
...
def _normalize_product(raw: dict, supplier: str) -> Optional[dict]:
    ...
    if supplier == "AIVERSE":
        ...
        price = _first(raw, "unit_price", "list_price", "price")
...
def _supplier_markup(supplier: str) -> Tuple[Decimal, Decimal]:
    ...
    if supplier == "ELITE":
        return ELITE_MARKUP_PERCENT, ELITE_MARKUP_USDT
    if supplier == "SHOPBOT":
        return SHOPBOT_MARKUP_PERCENT, SHOPBOT_MARKUP_USDT
    return AIVERSE_MARKUP_PERCENT, AIVERSE_MARKUP_USDT
...
def customer_price(x: dict) -> Decimal:
    ...
    Percent tracks reseller price changes automatically.
    Fixed USDT is an extra flat margin (optional, can be 0).
    """
...
def admin_suppliers(cid: Any) -> None:
    ...
    lines = [
        ...
        f"🔹 ShopAPI: <b>{'🟢 ON' if shop_on else '🔴 OFF'}</b> (key: {shop_key})",
        "",
        ...
        "",
        "⚠️ ShopAPI spends the <b>API key owner's wallet</b> on that shop — top up there first.",
    ]
    ...
    kb = [
        ...
        [
            {
                "text": "🔴 Turn OFF ShopAPI" if shop_on else "🟢 Turn ON ShopAPI",
